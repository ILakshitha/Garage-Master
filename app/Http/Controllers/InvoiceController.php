...
<?php
			...
			if (!empty($type == 0)) {
				...
				if ($emailformats->is_send == 0) {
					if ($invoice->save()) {
						...
						$search = array('{ system_name }', '{ Customer_name }', '{ service_title }', '{ service_date }','{currency_symbol}', '{ total_amount }', '{ Invoice }');
						$replace = array($systemname, $firstname, $title, $outdate,$currency_symbol,$final_grand_total,$invoice); 
						$email_content = str_replace($search, $replace, $mail_format);
		...
		if (!empty($type == 2)) {
            ...
            if ($emailformats->is_send == 0) {
				if ($invoice->save()) {
					...
					$search1 = array('{ invoice_number }');
					$replace1 = array($invoice_number);
					$mail_sub = str_replace($search1, $replace1, $mail_subjects);
					$currency_symbol =getCurrencySymbols();
					$final_grand_total = $invioce->grand_total;
					// $invoice = URL::to($filePath);
					$invoice =$invoice_number . '.pdf';
					$search = array('{ system_name }', '{ Customer_name }','{ invoice_date }','{currency_symbol}', '{ total_amount }', '{ Invoice }');
					$replace = array($systemname, $firstname,$dates,$currency_symbol,$final_grand_total,$invoice); 
					$email_content = str_replace($search, $replace, $mail_format);
	...
	public function serviceSend($id)
	{
		...
		// $whatsappURL = "https://api.whatsapp.com/send?phone=" . urlencode($mobile) . "&text=" . urlencode($message);
