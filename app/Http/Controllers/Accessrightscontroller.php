...
class Accessrightscontroller extends Controller
{
    ...
    public function storeAccessRights(Request $request)
    {
        ...
        $branchsetting = $request->branchsetting;
        $branchAdmin = $request->branchAdmin;
         // Add new modules
        ...
        if($id=='2'){
            ...
            // Ensure 'quotation_owndata' is included in the array if it's missing
            if (!in_array('quotation_owndata', $quotation)) {
                $quotation[] = 'quotation_owndata';
            }
        } 
        ...
        if (!empty($vehiclemodel)) {
            $data .= $this->store_regex($vehiclemodel) . ',';
        }
        if (!empty($colors)) {
        ...
        if (!empty($branch)) {
            $data .= $this->store_regex($branch) . ',';
        }
        if (!empty($branchsetting)) {
            $data .= $this->store_regex($branchsetting) . ',';
        }
        if (!empty($branchAdmin)) {
            $data .= $this->store_regex($branchAdmin) . ',';
        }
        // Add new modules processing
        if (!empty($notes)) {
            $data .= $this->store_regex($notes) . ',';
        }
        if (!empty($addons)) {
            $data .= $this->store_regex($addons) . ',';
        }
        if (!empty($howtovideo)) {
        ...
        if ($access_right) {
            return redirect('setting/accessrights/show')->with('message', 'Access Rights Updated Successfully');
        }
    ...
    {
        $roleData = Role::where([['status', '=', 1], ["id", '!=', 1]])->get();
