...
<?php }
    ...
    //     }
	// 	th {
    //         background-color:rgb(51, 52, 119);
    //         color: #ffffff;
    //         font-weight: bold;
    //         padding: 8px;
    //         text-align: center;
    ...
    // </style></head><body>';
	// $html .= '<h2>'. getNameSystem() . '</h2>';
    // $html .= '<table width="100%">';

    // if (!empty($tableData)) {
    //     $html .= '<tr>';
    //     foreach ($columnNames as $columnName) {
    //         $html .= '<th>' . htmlspecialchars($columnName, ENT_QUOTES, 'UTF-8') . '</th>';
    //     }
    //     $html .= '</tr>';
    ...
    //         $html .= '<tr>';
    //         foreach ($row as $value) {
	// 			$cleanValue = strip_tags($value); // Remove HTML tags
	// 			$cleanValue = html_entity_decode($cleanValue, ENT_QUOTES, 'UTF-8'); // Decode any HTML entities
    ...
    // // Extend default font config to use a font that supports Arabic
	...
	public function generate_pdf(Request $request)
{
    ...
    // Check language code for RTL/LTR
    $isArabic = getLangCode() === 'ar';
    
    // Generate HTML for the PDF content with conditional direction
    if ($isArabic) {
        // Arabic/RTL version
        $html = '<html><head><style>
            body { font-family: "dejavusans"; direction: rtl; text-align: right; }
        ...
        // English/LTR version
        $html = '<html><head><style>
            body { font-family: "dejavusans"; direction: ltr; text-align: left; }
            table, th, td {
                border: 1px solid black;
                border-collapse: collapse;
                padding: 5px;
