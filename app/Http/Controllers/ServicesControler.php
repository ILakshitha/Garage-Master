...
// \Log::info('Number Plate Debug:', ['number_plate' => $service]);

// Or more detailed
\Log::info('Service Debug Data:', [
    'service_id' => $service->id,
    'number_plate' => $service->number_plate,
    'customer_id' => $service->customer_id
                ...
                return [
                    ...
                    'customer_name' => $service->receiver_name ?: trans('message.Not Added'),
    ...
    // Apply custom filters for search
    private function applyCustomFilters($query, $request)
    {
        ...
        if ($daysFilter) {
            switch ($daysFilter) {
                case 'today':
                    $query->whereDate('tbl_services.service_date', now()->toDateString());
                    break;
                case 'yesterday':
                    $query->whereDate('tbl_services.service_date', now()->subDay()->toDateString());
                    break;
                case 'last_7_days':
                    $query->whereBetween('tbl_services.service_date', [now()->subDays(7)->toDateString(), now()->toDateString()]);
                    break;
                case 'last_30_days':
                    $query->whereBetween('tbl_services.service_date', [now()->subDays(30)->toDateString(), now()->toDateString()]);
                    break;
                case 'this_month':
                    $query->whereMonth('tbl_services.service_date', now()->month)
                          ->whereYear('tbl_services.service_date', now()->year);
                    break;
                case 'last_month':
                    $query->whereMonth('tbl_services.service_date', now()->subMonth()->month)
                          ->whereYear('tbl_services.service_date', now()->subMonth()->year);
                    break;
                case 'custom':
                    if ($startDate && $endDate) {
                        $query->whereBetween('tbl_services.service_date', [$startDate, $endDate]);
                    } elseif ($startDate) {
                        $query->whereDate('tbl_services.service_date', '>=', $startDate);
                    } elseif ($endDate) {
                        $query->whereDate('tbl_services.service_date', '<=', $endDate);
                    }
                    break;
            }
        }
    ...
    public function index()
    {
        ...
        // $last_order = DB::table('tbl_services')->get();

// With proper logging:
\Log::info('Last Order Debug:', [
    'last_order' => $last_order,
    'is_empty' => empty($last_order),
    'type' => gettype($last_order)
]);
        if (!empty($last_order)) {
        ...
        // ADD VEHICLES DATA - Role-based vehicle filtering
    if (isAdmin(Auth::User()->role_id)) {
        // Admin sees all vehicles
        $vehicles = DB::table('tbl_vehicles')->where('soft_delete', '=', 0)->get();
    } elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
        // Customer sees only their own vehicles
        $vehicles = DB::table('tbl_vehicles')
            ->where([['soft_delete', '=', 0], ['customer_id', '=', Auth::User()->id]])
            ->get();
    } else {
        // Employees/Branch Admin see vehicles from their branch
        $vehicles = DB::table('tbl_vehicles')
        ...
        return view('service.add', compact('employee', 'customer', 'code', 'country', 'onlycustomer', 'vehical_brand', 'vehical_type', 'fuel_type', 'color', 'model_name', 'tbl_custom_fields', 'branchDatas', 'repairCategoryList', 'vehicles'));
    }
    ...
    public function vehicleadd(Request $request)
    {
        ...
        try {
            ...
            \Log::info('Vehicle details:', ['request' => $request->all(), 'vehical' => $vehical]);
            ...
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'vehicle_data' => [
                        'id' => $vehical->id,
                        'number_plate' => $vehical->number_plate,
                        'model_name' => $vehical->modelname,
                        'brand' => $vehical->vehiclebrand_id
                    ]
                ]);
            }

            return back()->with('vehicle', $vehical->id);
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'An error occurred while adding the vehicle. Please try again.',
                    'error' => $e->getMessage()
                ], 500);
            }
            
            return back()->with('error', 'An error occurred while adding the vehicle.');
        }
    }
...
//end new added logic for storing data relationally
        ...
        try {
            ...
            if ($emailformats->is_send == 0) {
                if ($tbl_jobcard_details->save()) {
                    ...
                    $messageBody = "Thank you for bringing your vehicle to our service center.Your Jobcard Number is".$servicedetails->job_no."created on".$servicedetails->service_date;
    ...
    public function store(Request $request)
    {
         ...
         // Add debugging right at the beginning
    \Log::info('Service store method called with customer_id:', [
        'customer_id' => $request->customer_id,
        ...
        $job = $request->jobno;
        $vehicalname = $request->Vehiclename;
        // Remove the incorrect Customername assignment - we use customer_id instead
        ...
        // Debug vehicle and customer ID assignment
        \Log::info('Vehicle and Customer ID Assignment Debug:', [
            'raw_vehiclename' => $request->Vehiclename,
            'assigned_vehicalname' => $vehicalname,
            'raw_customer_id' => $request->customer_id,
            'assigned_customername' => $Customername,
            'vehiclename_is_null' => is_null($vehicalname),
            'vehiclename_is_empty' => empty($vehicalname),
            'vehiclename_is_numeric' => is_numeric($vehicalname)
        ]);
        
        // Remove the conflicting line that was overwriting customer_id
        $date = $request->date;
        $kms = $request->kms;
    ...
    \Log::info('Image processing debug:', [
        ...
        'image_count' => is_array($image) ? count($image) : 'not_array'
    ]);
        $image = $request->image;
        ...
        if ($available) {
            ...
            $available->receiver_name = $request->receiver_name;
            
            // Add debugging for customer_id
            \Log::info('Updating existing service with data:', [
                'service_id' => $available->id,
                'customer_id' => $Customername,
                'vehicle_id' => $vehicalname,
                'receiver_name' => $request->receiver_name,
                'job_no' => $request->notepad,
            ]);
            ...
            $available->notepad = $request->notepad;
            ...
            // die($customer);
            $toEmail = $customer ? $customer->email : "noreply@example.com";
            // Save Notes data
            if ($available->save()) {
            ...
            $services->receiver_name = $request->receiver_name;
            $services->notepad = $request->notepad;
            
            // Add debugging for new service creation
            \Log::info('Creating new service with data:', [
                'customer_id' => $Customername,
                'vehicle_id' => $vehicalname,
                'receiver_name' => $request->receiver_name,
                'job_no' => $job,
                'customer_id_from_request' => $request->customer_id,
                'vehicle_id_from_request' => $request->Vehiclename,
                'customer_id_is_null' => is_null($Customername),
                'customer_id_empty' => empty($Customername),
                'vehicle_id_is_null' => is_null($vehicalname),
                'vehicle_id_empty' => empty($vehicalname),
                "service notepad" => $request->notepad,
            ]);
           ...
           // dd($request->all());
            ...
            // Handle case where customer might not exist or customer_id is null
            \Log::info('Finding customer for new service:', [
        ...
        if (!empty($image)) {
            $files = $image;
             \Log::info('Files to process:', ['file_count' => count($files)]);
        ...
        //  $model_id = getModel_id($vehical->modelname);
         $model_id = "1234333"; //temp id for testing
    ...
    public function serviceedit($id)
    {
        ...
        return view('/service/edit', compact('service','services','markers','services_kms', 'vehical', 'employee', 'customer', 'regi_no', 'tbl_custom_fields', 'washbayPrice', 'branchDatas', 'repairCategoryList', 'images1','receiver_name'));
    }
    ...
    public function serviceupdate(Request $request, $id)
    {
        ...
        $vehicalname = $request->vehicalname; // Changed from vehical_id to vehicalname
        $kms = $request->kms;
        // Fix: Use the correct field name from the form  
        $Customername = $request->Customername; // Changed from customer_id to Customername
        $details = $request->details;
        $car_details = $request->car_details;
        $markers=$request->input('markers');
        //dd($markers);
        ...
        if ($receiver_name !== null) {
            $available->receiver_name = $receiver_name;
        }
        ...
        $customer = User::find($available->customer_id);
        // Handle case where customer might not exist or customer_id is null
        \Log::info('Finding customer for service update:', [
            'customer_id' => $available->customer_id,
            'customer_found' => !is_null($customer),
            'customer_email' => $customer ? $customer->email : 'N/A'
        ]);
        $toEmail = $customer ? $customer->email : "noreply@example.com";
        ...
        if (!empty($vehical) && isset($vehical->modelname)) {
            $model_id = getModel_id($vehical->modelname);
        } else {
            \Log::warning('Vehicle not found or missing modelname for vehicle_id: ' . $veh_id);
            $model_id = null; // or set a default value
        }
        ...
        if (!empty($sale_regi)) {
            ...
            // Add validation to prevent SQL error when vehicalname is empty
            if (!empty($vehicalname) && !is_null($vehicalname)) {
                DB::update("update tbl_vehicles set registration_no = '$reg_no' where id = $vehicalname");
            } else {
                \Log::warning('Vehicle ID is empty or null, skipping vehicle registration update');
            }
        }
        $logo = DB::table('tbl_settings')->first();
        $inspection_points_library_data = DB::table('inspection_points_library')->get();
...
<?php
    ...
    public function view($id)
    {
        ...
        $cus_id = $service->customer_id;
        $customer = DB::table('users')->where('id', $cus_id)->first();
        $vah_id = $service->vehicle_id;
        $tbl_sales = DB::table('tbl_sales')->where('vehicle_id', $vah_id)->first();
        $images1 = DB::table('tbl_service_images')->where('service_id', '=', $id)->get()->toArray();
            ...
            try {
                ...
                if ($emailformats->is_send == 0) {
                    if ($customer->save()) {
                        ...
                        $systemname = $logo->system_name;
                        $systemlink = URL::to('/');
                        $messageBody = "Welcome to " . $systemname ."Your account is created successfully. Your login details are as follows: Email:\n " . $email . "\n Password: " . $password."\nLogin Url :".$systemlink;
