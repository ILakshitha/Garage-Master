        ...
        <div class="row">
          ...
          <div class="col-sm-7 col-sm-offset-3 col-md-7 col-md-offset-2 main content-start">
            ...
            @php
                ...
                $isMobile = \Illuminate\Support\Facades\DB::table('tbl_settings')->value('is_mobile');
            @endphp
          @if($smsAddonExists && $isMobile == 1)
            {{-- Mobile/OTP Login --}}
            
            <div class="col-md-6 mb-2">
                <h5>{{trans('message.Mobile OTP Login')}}</h5>
                @if(session('otp_sent'))
                    {{-- OTP Verify Form --}}
                    <form method="POST" action="{{ route('login.otp.verify.submit') }}">
                        @csrf
                        <input type="text" name="otp" placeholder="{{trans('message.Enter OTP')}}" required>
                ...
                @else
                    ...
                    <form method="POST" action="{{ route('login.otp.send') }}">
                        @csrf
                        <input type="text" name="mobile_no" placeholder="{{trans('message.Enter Mobile Number')}}" required>
                        <button type="submit" class="otpButton">{{trans('message.Send OTP')}}</button>
                    </form>
                @endif
            </div>
                ...
                if (authCode) {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "client_id": "da51d9ab-19c9-48b2-bff6-85e5b0c18a69",
                            "client_secret": "2563eef1-33f3-4d56-adc8-87dec1092658",
                            "code": authCode,
                            "access_token": access_token, 
                            // "instance_id": "f7271218-5c1e-4621-b461-3e2a8af56c02", 
                            "email": email,
                            "password": password,  
                        ...
                        success: function(data) {  
                            ...
                            $("#tokenData").html("Access Token: " + data.access_token); // Display access token
                        }
                    });
                } else {
