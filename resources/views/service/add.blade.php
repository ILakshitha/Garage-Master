...
function setupDropZone() {
 ...
 $(document).ready(function() {
          // Custom modal handler for data-bs-toggle="new-modal"
    $('button[data-bs-toggle="new-modal"]').on('click', function(e) {
        e.preventDefault();
        
        // Get the target modal ID from data-bs-target
        var targetModal = $(this).attr('data-bs-target');
        
        // Optional: Add customer validation before opening modal
        var customerId = $('#customer_id').val();
        
        if (!customerId || customerId === '') {
            // Show error if no customer selected
            swal({
                title: "{{ trans('message.Alert') }}",
                text: "{{ trans('Please select customer first!') }}",
                icon: "warning",
                buttons: {
                    cancel: "{{ trans('message.OK') }}",
                },
                dangerMode: true,
            });
            return false;
        ...
        // Optional: Set customer ID in hidden field when modal opens
        $('.hidden_customer_id').val(customerId);
        
        console.log('Custom modal opened:', targetModal);
        console.log('Customer ID:', customerId);
    });
    
    // Also handle the modal show event
    $('#vehiclemymodel').on('show.bs.modal', function () {
        var customerId = $('#customer_id').val();
        console.log("Modal opening with Customer ID:", customerId);
        $('.hidden_customer_id').val(customerId);
    });
...
$(document).ready(function() {
    ...
    setTimeout(function() {
        // Remove any existing jQuery validation from this form
        if (typeof $.fn.validate !== 'undefined') {
            $('#add_vehi').removeData('validator');
            $('#add_vehi').off('.validate');
        }
        
        // Ensure the event is bound after DOM is ready and remove any existing handlers
        $('#add_vehi').off('submit.vehicle').on('submit.vehicle', function(e) {
            e.preventDefault(); // Prevent default form submission
            e.stopImmediatePropagation(); // Stop all other event handlers
            
            console.log('Vehicle form AJAX submission triggered'); // Debug log
            
            // Disable submit button immediately to prevent multiple submissions
            var submitBtn = $(this).find('button[type="submit"]');
            if (submitBtn.prop('disabled')) {
                console.log('Submit button already disabled, returning'); // Debug log
            ...
            var isValid = true;
            var vehicleType = $('#vehical_id1').val();
            var numberPlate = $('#number_plate').val();
            var vehicleBrand = $('#vehicabrand1').val();
            var modelName = $('#modelname1').val();
            var customerId = $('.hidden_customer_id').val();
            
            console.log('Form values:', {
                vehicleType, numberPlate, vehicleBrand, modelName, customerId
            }); // Debug log
            
            // Validate required fields
            if (!vehicleType) {
                $('#errorlvehical_id1').text('Please select vehicle type.');
                isValid = false;
            }
            
            if (!numberPlate.trim()) {
                $('#npe').text('Number plate is required.');
                isValid = false;
            }
            
            if (!vehicleBrand) {
                $('#errorlvehicabrand1').text('Please select vehicle brand.');
                isValid = false;
            }
            
            if (!modelName) {
                $('#errorlmodelname1').text('Please select model name.');
                isValid = false;
            }
            ...
            console.log('Validation passed, making AJAX request'); // Debug log
                ...
                success: function(response) {
                    ...
                    if(response.success) {
                        ...
                        swal({
                            ...
                            $('#vehiclemymodel').modal('hide'); // Close modal
                        ...
                        if(response.errors) {
                            $.each(response.errors, function(field, messages) {
                                var errorElement = $('#errorl' + field + '1');
                        ...
                        swal({
                            title: "Error!",
                            text: response.message || "Failed to add vehicle. Please check the form and try again.",
                            icon: "error",
                            button: "OK",
                        });
                    }
                ...
                error: function(xhr, status, error) {
                    ...
                    if(xhr.status === 422) {
                        ...
                        swal({
                            ...
                            text: "Please check the form and correct any errors.",
                            icon: "warning",
                            button: "OK",
                        });
                    } else {
        ...
        // Also handle direct button clicks as a backup
        $('#add_vehi button[type="submit"]').off('click.vehicle').on('click.vehicle', function(e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            $('#add_vehi').trigger('submit.vehicle');
            return false;
        });
        
    }, 100); // Small delay to ensure other scripts have initialized
});
</script>
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('step2Btn').addEventListener('click', function(e) {
        // Get owner and vehicle values
        var owner = document.getElementById('customer_id').value;
        ...
        if (!vehicle) {
            vehicleError.textContent = 'Please select vehicle.';
            hasError = true;
        }
        ...
        // If no error, allow tab switch or navigation (your existing logic)
        // You may need to add code here to show step 2 content if using tabs
    });
});
</script>
...
<style>
    ...
    .tab-menu {
        ...
        /* Optional: Add a bottom border for the container */
    }
    ...
    .select2-container--default .select2-results>.select2-results__options {
        ...
        /* For modern browsers, make scrollbar thinner */
    }

    @if(getLangCode()==='ar') .select2-container--default .select2-selection--single .select2-selection__arrow {
...
<div class="right_col" role="main">
    ...
    <div class="tab-container ">
        <div class="tab-menu">
            <button class="tab-link active">{{trans('message.Step 1: Add Service')}}</button>
                ...
                <div class="x_content">
                    <div class="panel panel-default">
                        ...
                        <form id="ServiceAdd-Form" method="post" action="{{ url('/service/store') }}" enctype="multipart/form-data" class="form-horizontal upperform serviceAddForm" border="10">
                            ...
                            <!-- <div class="row row-mb-0">
                                <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 mt-3">
                    ...
                    <div class="row">
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 ">
                            ...
                            <div class="col-md-2 col-lg-2 col-xl-2 col-xxl-2 col-sm-2 col-xs-2 addremove">
                                <!-- <button type="button" class="btn btn-outline-secondary btn-sm" onclick="$('#myModal').modal('show');">
                                    +
                                </button> -->
                            </div>
                        </div>       
                        ...
                        <!-- Receiver Name Field -->
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 ">
                            <label class="control-label col-md-4 col-lg-4 col-xl-4 col-xxl-4 col-sm-4 col-xs-4" for="receiver_name">{{ trans('message.Receiver Name') }}</label>
                            <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                                <input type="text" id="receiver_name" name="receiver_name" placeholder="{{ trans('message.Enter Receiver Name') }}" class="form-control" maxlength="100" value="{{ old('receiver_name') }}">
                                <span class="color-danger" id="errorlreceiver_name"></span>
                            </div>
                        </div>
                    ...
                    <div class="row row-mb-0">
                        ...
                        <div id="dvCharge" class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 has-feedback {{ $errors->has('Charge') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                                ...
                                <input type="text" id="charge_required" name="charge" class="form-control fixServiceCharge" placeholder="{{ trans('message.Enter Service Charge') }}" maxlength="8">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row row-mb-0">
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                            ...
                            <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                                <input type="text" min='0' id="kms" name="kms" value="" maxlength="10" class="form-control" required>
                            </div>
                        </div>
                    ...
                    <div class="row row-mb-0">
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                            <label class="control-label col-md-4 col-lg-4 col-xl-4 col-xxl-4 col-sm-4 col-xs-4 pt-0" for="first-name">{{ trans('message.Select Multiple Images') }} <label class="color-danger"></label></label>
                            <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                               
                    <input type="file" name="image[]" class="form-control imageclass" id="images" onchange="preview_images();" data-max-file-size="5M" multiple accept="image/*" />
                    <!-- /*Image Preview Container*/ -->
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 mt-3 d-block">
                            <div id="car-image-area" style="position: relative; width:100%;max-width:433px;">
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 mt-3 d-block">
                            ...
                            <div class="col-md-12 col-lg-12 col-xl-12 col-xxl-12 col-sm-12 col-xs-12 mt-3">
                                <textarea class="form-control mb-2" name="car_details" id="car_details" rows="5">{{ old('car_details') }}</textarea>
                            </div>
                            <canvas id="car-image-canvas"></canvas> <!-- Canvas for drawing markers -->
                    ...
                    <!-- Notepad Section -->
                    <div class="row col-md-12 col-lg-12 col-xl-12 col-xxl-12 col-sm-12 col-xs-12 mt-3">
                        <label class="control-label col-md-2 col-lg-2 col-xl-2 col-xxl-2 col-sm-2 col-xs-2" for="notepad">{{ trans('Notepad') }}</label>
                        <div class="col-md-10 col-lg-10 col-xl-10 col-xxl-10 col-sm-10 col-xs-10">
                            <textarea id="notepad" name="notepad" class="form-control" placeholder="{{ trans('message.Enter notes...') }}">{{ old('notepad') }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="row mt-3">
                            <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 form-group has-feedback">
                                ...
                                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8 gender ps-0">
                                    <input type="radio" class="gender" name="gender" value="0">{{ trans('message.Male') }}
...
<div class="modal" id="vehiclemymodel" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="#" enctype="multipart/form-data" class="form-horizontal upperform" id="add_vehi" onsubmit="return false;" novalidate>
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="branch_id_vehicle" value="{{ auth()->user()->branch_id ?? 1 }}">
                    <input type="hidden" class="hidden_customer_id" name="customer_id" value="">
                    <div class="row">
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                            ...
                            <div class="col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                                <select class="form-control select_vehicalbrand form-select" id="vehicabrand1" name="vehicabrand" url="{!! url('/vehicle/vehicalmodelfrombrand') !!}">
                                    ...
                                    <option value="{{ $vehical_brands->id }}">{{ $vehical_brands->vehicle_brand }}
                                    </option>
                                    @endforeach
                                    @endif
                                </select>
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                            ...
                            <div class="col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                                <select class="form-control model_addname form-select" id="modelname1" name="modelname" required>
                                    ...
                                    <option value="{{ $model_names->model_name }}">{{ $model_names->model_name }}
                                    </option>
                                    @endforeach
                                    @endif
                    ...
                    <div class="row mt-2">
                        ...
                        <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                            ...
                            <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8 date">
                                <input type="date" name="dom" id="dom1" class="form-control datepicker1" placeholder="<?php echo getDatepicker(); ?>" onkeypress="return false;" />
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    let markerCount = 0; // Initialize a counter for marker numbers

    document.getElementById('car-image-area').addEventListener('click', function(event) {
        // Prevent creating markers when clicking on existing markers
        ...
        const x = event.clientX - rect.left; // X-coordinate relative to the container
        const y = event.clientY - rect.top; // Y-coordinate relative to the container

        // Increment the marker count
        markerCount++;

        // Create a new marker container to hold the number and the red dot
        const markerContainer = document.createElement('div');
        markerContainer.className = 'marker-container';
        markerContainer.style.position = 'absolute';
        markerContainer.style.left = `${x}px`; // Accurate position of the container
        markerContainer.style.top = `${y}px`; // Accurate position of the container

        // Create a number label
        const markerNumber = document.createElement('span');
        markerNumber.className = 'marker-number';
        markerNumber.innerText = markerCount; // Set the number
        markerNumber.style.position = 'absolute';
        markerNumber.style.left = '-5px'; // Offset the number to the left of the dot
        markerNumber.style.top = '10%';
        markerNumber.style.transform = 'translateY(-50%)'; // Vertically center the number
        markerNumber.style.color = 'black'; // Set the number color
        markerNumber.style.fontWeight = 'bold';

        // Create the red dot marker
        const marker = document.createElement('div');
        marker.className = 'damage-marker';
        marker.style.position = 'absolute';
        marker.style.width = '12px'; // Size of the red dot
        marker.style.height = '12px';
        marker.style.borderRadius = '50%';
        marker.style.backgroundColor = 'red'; // Default color for the dot
        marker.style.left = '0'; // Position the red dot relative to the container
        marker.style.top = '0';
        marker.title = `Damage Marker ${markerCount}`;

        // Add double-click listener to toggle marker color
        marker.addEventListener('dblclick', function(event) {
            event.stopPropagation(); // Prevent triggering parent events
            if (marker.style.backgroundColor === 'red') {
                marker.style.backgroundColor = 'red';
                marker.title = `Confirmed Marker ${markerCount}`;
            } else {
                markerContainer.remove(); // Remove the entire container (marker and number)
                updateMarkers(); // Update marker data after removal
            }
        });

        // Add right-click listener to remove marker
        ...
        // Append the number and the marker to the container
        markerContainer.appendChild(markerNumber);
        markerContainer.appendChild(marker);

        // Append the container to the car image area
        document.getElementById('car-image-area').appendChild(markerContainer);

        // Update marker data after adding a new marker
        updateMarkers();
    });

    // Function to update the hidden input field with marker data
    function updateMarkers() {
        const markerContainers = document.querySelectorAll('.marker-container');
        const markers = [];

        // Reset markerCount if no markers exist
        if (markerContainers.length === 0) {
            markerCount = 0;
        }

        markerContainers.forEach((container, index) => {
            const x = parseFloat(container.style.left);
            const y = parseFloat(container.style.top);

            const marker = container.querySelector('.damage-marker');
            const markerNumber = container.querySelector('.marker-number');

            // Update the marker number to maintain sequential order
            markerNumber.innerText = index + 1;
            marker.title = `Damage Marker ${index + 1}`;

            markers.push({
                x: x,
                y: y,
                number: markerNumber.innerText,
                color: marker.style.backgroundColor
            });
        });

        // Update the hidden input field with marker JSON
        document.getElementById('markers').value = JSON.stringify(markers);
    }

    // Function to save the image with markers
    document.getElementById('save-image').addEventListener('click', function() {
        ...
        if (!carImage.complete) {
            alert('Image is not fully loaded. Please try again.');
            return;
        }
        ...
        // Scale markers proportionally
        const scaleX = canvas.width / carImage.width;
        const scaleY = canvas.height / carImage.height;

        // Draw the car image onto the canvas
        ctx.drawImage(carImage, 0, 0, canvas.width, canvas.height);

        // Draw each marker on top of the image
        const markerContainers = document.querySelectorAll('.marker-container');
        markerContainers.forEach(container => {
            const x = parseFloat(container.style.left) * scaleX;
            const y = parseFloat(container.style.top) * scaleY;
            ...
            ctx.fillStyle = marker.style.backgroundColor;
            ctx.fill();

            // Draw the number near the red dot
            ctx.fillStyle = 'black';
            ctx.font = 'bold 12px Arial';
            ctx.fillText(markerNumber.innerText, x - 15, y + 5);
        });
        ...
        link.href = imageURL;
        link.download = 'car_with_markers.png';
        link.click();
    });
</script>

<!-- Add CKEditor for notepad -->
<script src="{!! URL::asset('public/vendor/ckeditor/ckeditor.js') !!}"></script>

<script>
$(document).ready(function() {
    // Initialize CKEditor for notepad
    CKEDITOR.replace('notepad', {
        toolbar: [{
            name: 'styles',
            items: ['Bold', 'Italic']
        },
...
<style>
    ...
    .marker-number {
        ...
        position: absolute;
        left: -12px;
        /* Offset the number to the left of the dot */
        top: 50%;
...
<script>
    $(document).ready(function() {
        ...
        $('#cust_id').select2({
            placeholder: "{{ trans('message.Select Customer') }}",
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('body').on('click', '.addvehicleservice', function(event) {
            ...
            // When opening the vehicle modal, set the customer ID
            $('#vehiclemymodel').on('show.bs.modal', function () {
                var customerId = $('.select_vhi').val(); // Get selected customer ID
                        ...
                        success: function(data) {
                            ...
                            if (newd == "01") {
                                ...
                                $('.vehical_brand_class').append('<tr class=" row mx-1 ' + classname +
                                    ...
                                    data +
                                    'deletevehicalbrand="{!! url('
                                    vehicle / vehicalbranddelete ') !!}" class="btn btn-danger text-white border-0 deletevehiclebrands"><i class="fa fa-trash" aria-hidden="true"></i></button></a></td><tr>'
                                );
        ...
        $('#cust_id').on('change', function() {
            ...
            var supplierValue = $('select[name=customer_id]').val();
        ...
        $('body').on('click', '.serviceSubmitButton', function(e) {
            $('#ServiceAdd-Form input, #ServiceAdd-Form select, #ServiceAdd-Form textarea').each(
                ...
                function(index) {
                    ...
                    if (input.attr('name') == "customer_id" || input.attr('name') ==
        ...
        $('body').on('keyup', '.washbay_charge_textbox', function() {
            ...
            if (washbayVal != "") {
                if (!washbayVal.match(/^(0|[1-9][0-9]*)$/).length) {
                    $(this).val("");
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        ...
                        success: function() {
                            $('.del-' + colorid).remove(); // Remove the row from the table
                            $(".repair_category option[value='" + colorid + "']").remove(); // Remove the option from the select dropdown
    ...
    $(document).ready(function() {
        ...
        $('body').on('keyup', '.mot_charge_textbox', function() {
            ...
            if (motVal != "") {
                if (!motVal.match(/^(0|[1-9][0-9]*)$/).length) {
                    $(this).val("");
