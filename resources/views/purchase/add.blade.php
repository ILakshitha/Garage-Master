    ...
    <div class="nav_menu">
      <nav>
        <div class="nav toggle">
          <a id="menu_toggle">&nbsp;<i class="fa fa-bars sidemenu_toggle"></i></a><span class="titleup"><a href="{!! url('/purchase/list') !!}" id=""><i class=""><img src="{{ URL::asset('public/supplier/Back Arrow.png') }}" class="back-arrow"></i><span class="titleup">
              ...
              <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 ">
                ...
                <div class="col-md-6 col-lg-6 col-xl-6 col-xxl-8 col-sm-8 col-xs-8 form-group">
                  <select class="form-control select_supplier_auto_search s_product w-100 form-select" name="s_name" id="supplier_select" url="{!! url('purchase/add/getrecord') !!}" s_url="{!! url('purchase/add/getsupplierproduct') !!}" required>
                    <option value="" class="sp_placeholder">{{ trans('message.select supplier') }}</option>
                    @if (!empty($supplier))
                    @foreach ($supplier as $suppliers)
                    <option value="{{ $suppliers->id }}">{{$suppliers->name}} {{$suppliers->lastname}} - {{ $suppliers->company_name }}</option>
                    @endforeach
                    @endif
                  </select>
              ...
              <table class="table table-bordered adddatatable" id="tab_taxes_detail" align="center">
                ...
                <tbody>
                  <tr id="row_id_1">
                    ...
                    <td>
                      <input required type="text" name="product[price][]" class="product form-control prices price_1" value="" id="price_1" style="width:100%;" row_id="1" style="width:100%;" readonly>
                    </td>
                    <td>
...
<script>
  ...
  // Use a global variable to track the last assigned row ID
  if (typeof window.lastAssignedRowId === 'undefined') {
    window.lastAssignedRowId = -1;
  }

  var row_len = jQuery(".row_number").length;
  if (row_len > 0) {
    var num = jQuery(".row_number:last").val();
    var potentialRowId = parseInt(num) + 1;
    
    // Ensure the new row ID is always greater than the last assigned ID
    window.lastAssignedRowId = Math.max(window.lastAssignedRowId + 1, potentialRowId);
  } else {
    // If no rows exist, start from 0 or the last assigned ID + 1
    window.lastAssignedRowId = Math.max(window.lastAssignedRowId + 1, 0);
  }
  var row_id = window.lastAssignedRowId;
