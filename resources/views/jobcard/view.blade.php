...
.searchable-single + .select2-container .select2-selection--single {
    width: 110px !important;
}
.searchable-single +.select2-container--default .select2-selection--single .select2-selection__arrow {
    right: 7px !important;
}
.select2-dropdown{
                            ...
                            <form method="post" action="{!! url('/jobcard/store') !!}" class="addJobcardForm">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-5 col-lg-5 col-xl-5 col-xxl-5 col-sm-5 col-xs-5">
                                        ...
                                        <div class="row row-mb-0">
                                            <label class="control-label jobcardmargintop col-md-4 col-lg-4 col-xl-4 col-xxl-4 col-sm-4 col-xs-4">{{ trans('message.Next Service Date') }}
                                                <label class="color-danger">*</label></label>
                                            <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8 date ">
                                                {{-- <span class="input-group-addon"><i
                                                                class="glyphicon glyphicon-calendar fa fa-calendar"></i></span> --}}
                                ...
                                <div class="row panel-group">
                                    ...
                                    <select id="searchable-select" class="form-control" multiple="multiple" data-placeholder="{{ trans('message.Select Checkpoints') }}">
    ...
    <?php } else {
        foreach ($data as $datas) { ?>
            ...
            <tr class="obs_point_data" id="<?php echo 'row_id_delete_' . $i; ?>">
                ...
                <td>
                    <input type="number" name="product2[service_charge][]" value="<?php echo $datas->service_charge ?? 0;?>" class="form-control charge charge_{{ $i }}" row_id="{{ $i }}" maxlength="8">
                </td>
                ...
                <td>
                   ...
                   {{ trans('message.Yes:') }} <input type="radio" name="yesno_[]<?php echo $i; ?>" class="yes_no" value="1" <?php if ($datas->chargeable == 1) { echo 'checked'; } ?> style="height:13px; width:20px; margin-right:5px;">
                    ...
                    <div>
                        {{ trans('message.No:') }}
                     <input type="radio" name="yesno_[]<?php echo $i; ?>" class="yes_no" value="0" <?php if ($datas->chargeable == 0) { echo 'checked'; } ?> style="height:13px; width:20px;">
                    </div>
                    </td>
        ...
        <?php } 
                                ...
                                <div class="row">
                                    ...
                                    <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 my-1 mx-0 pe-3" style="@if(getLangCode() == 'ar') width:100%; @endif">
...
<!-- /page content -->
<script>// Function to populate prices for pre-selected products on page load
function populateExistingPrices() {
    $('#main_data_points tbody tr.obs_point_data').each(function(index) {
        var $row = $(this);
        var row_id = index + 1; // or extract from existing elements
        
        // Get the selected product ID
        var product_id = $row.find('select[name="product2[product_id][]"]').val();
        var serviceCharge = parseFloat($row.find('input[name="product2[service_charge][]"]').val()) || 0;
        
        // If product is selected, fetch its price
        if (product_id && product_id !== '') {
            $.ajax({
                url: "{{ route('jobcard.getprice') }}", // Make sure this route exists
                type: 'GET',
                data: { 
                ...
                success: function(response) {
                    if (response && response.length > 0) {
                        ...
                        // Update price field
                        $row.find('input[name="product2[price][]"]').val(price);
                        
                        // Recalculate total
                        var qty = parseFloat($row.find('input[name="product2[qty][]"]').val()) || 1;
                        var total = (price * qty) + serviceCharge;
                        $row.find('input[name="product2[total][]"]').val(total.toFixed(2));
                    }
                },
                error: function(xhr, status, error) {
                    console.log('Error fetching price for product ID ' + product_id + ':', error);
                }
            });
        }
    });
...
<script>
    $(document).ready(function () {
            ...
            selectedOptions.forEach((option) => {
                ...
                const newRow = `
                    <tr id="row-${categoryId}" class="obs_point_data">
                        ...
                        <td>
                            <select name="product2[product_id][]" class="form-control product_ids searchable-single" data-row-index="${rowIndex}" required>
             ...
             $(`.searchable-single[data-row-index="${rowIndex}"]`).each(function () {
                if (!$(this).hasClass("select2-hidden-accessible")) {
                    $(this).select2({
                        placeholder: "{{ trans('message.Select Product') }}",
                        width: '100%',
                        allowClear: true,
        ...
        // $('.searchable-single').each(function () {
        //     if (!$(this).hasClass("select2-hidden-accessible")) {
        //         $(this).select2({
          ...
          $('.searchable-single').each(function () {
            if (!$(this).hasClass("select2-hidden-accessible")) {
                $(this).select2({
                    ...
                    width: '100%',
                    allowClear: true,
                    minimumInputLength: 0, // ⚡ Only start searching after 2 characters
                    sorter: data => data.sort((a, b) => a.text.localeCompare(b.text)), // optional: sort alphabetically
                    language: {
                        searching: function () {
                            return "Searching...";
                        },
        ...
        function updateSelect2Placeholder() {
            ...
            const placeholderText = numSelected > 0 ? `${numSelected} selected` : $('#searchable-select').data('placeholder');
...
<script>
    $(document).ready(function() {
        ...
        $('.datepicker').datetimepicker({
            ...
            @if(getLangCode() == 'ar')
                setTimeout(function () {
                    // Target the open datetimepicker and apply RTL style
                    $('.datetimepicker.dropdown-menu').css({
                        'right': '423px',
                        'left': '666px'  // override left if needed
                    });
                }, 10);
            @endif
        });
